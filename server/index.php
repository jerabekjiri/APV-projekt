...
$app->get('/api/user/relation/{user1}/{user2}', function ($request, $response, $args) {

    $user1 = $args['user1'];
		$user2 = $args['user2'];

		$relation = Db::GET_RELATION($user1, $user2);

		return $response->getBody()->write(json_encode($relation));
});
...
$app->get('/api/user/{id}/friends', function ($request, $response, $args) {
...
$app->post('/api/user/add-friend', function ($request, $response) {

	$data = $request->getParsedBody();


	Db::ADD_FRIEND($data['user1'], $data['user2']);

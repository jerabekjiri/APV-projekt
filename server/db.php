...
class Db{
  ...
  function GET_USER_BY_NAME($href)
  {
    $sql= "SELECT user_id, name, email, avatar, information, href FROM users WHERE href = :href";
    $stmt = Db::$pdo->prepare($sql);
    $stmt->execute(array( 'href' => $href));
    $result = $stmt->fetch();
  ...
  {
    $sql= "SELECT relations.relation_id, users.name, users.href
           FROM relations
           LEFT JOIN users ON users.user_id = relations.user2_id
  ...
  {
    $stmt = Db::$pdo->prepare("INSERT INTO relations (user_id, user2_id) VALUES (:user_id, :user2_id)");
    $stmt->execute(array(
                    'user_id' => $user1,
                    'user2_id' => $user2
                    ));
  ...
  function GET_RELATION($user1, $user2)
  {
    $stmt = Db::$pdo->prepare("SELECT EXISTS (SELECT * FROM relations WHERE user_id= :user1 AND user2_id= :user2)");
    $stmt->execute(array(
                    'user1' => $user1,
                    'user2' => $user2));
    $result = $stmt->fetch();
